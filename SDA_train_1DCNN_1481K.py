...
from SDA_data_utils_cwru import (
    load_cwru_signals,
...
# -------------------------------
SOURCE_KEY = "12K1HP"
TARGET_KEY = "48K1HP"   # pick from cwru
...
tgt_signals, tgt_labels, tgt_rates = load_cwru_signals(TARGET_KEY)
...
def train_supervised_da_kfold(src_signals, src_labels,
    ...
    base_dir = f"Results_SupervisedDA_From_{SOURCE_KEY}_To_{TARGET_KEY}_{timestamp}"
    ...
    for train_idx, val_idx in kfold.split(X_src, y_src):
        ...
        Xt_train, Xt_test, yt_train, yt_test = train_test_split(X_tgt, y_tgt, test_size=0.2, random_state=SEED+fold, stratify=y_tgt)
        # build model
