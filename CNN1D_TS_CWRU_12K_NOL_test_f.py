...
from CWRU_data import ImportData_CWRU_LHP1_12K
...
Data = ImportData_CWRU_LHP1_12K()
for i in range(1):
    Data[i] = resample_48K_to_12K(Data[i])
# Data[0] = resample_48K_to_12K(Data[0])
# Data[0] = data_12K
...
for fold, (train, test) in enumerate(kfold.split(X_1D_train, y_label_train)):    
  ...
  train_loss, train_accuracy = CNN_1D_best_model.evaluate(X_1D_train, y_1D_train)
  accuracy_train.append(train_accuracy)

  # Evaluate the accuracy of the model on the validation set 
  # val_loss, val_accuracy = CNN_1D_best_model.evaluate(X_1D_train[test], y_1D_train[test])
  val_loss, val_accuracy = CNN_1D_best_model.evaluate(X_1D_test, y_1D_test)
  accuracy_val.append(val_accuracy)

  # Evaluate the accuracy of the model on the test set 
  # test_loss, test_accuracy = CNN_1D_best_model.evaluate(X_1D_test, y_1D_test) 
  test_loss, test_accuracy = CNN_1D_best_model.evaluate(Input_1D, Y_CNN)
...
pred_label = np.column_stack((pred_label1, pred_label2, pred_label3, pred_label4, pred_label5, target_label))
pred_label_trn = np.column_stack((pred_label1_trn, pred_label2_trn, pred_label3_trn, pred_label4_trn, pred_label5_trn, target_label_trn))
...
"""
np.savetxt("pred_CNN1D_V4_bm.csv", pred_label, delimiter=",", fmt="%d")
np.savetxt("pred_trn_CNN1D_V4_bm.csv", pred_label_trn, delimiter=",", fmt="%d")
